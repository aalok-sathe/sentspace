...
def load_df_from_directory(
    ...
    dfs = pd.DataFrame()
    glob = [
        *(Path(directory) / module).resolve().glob(f"sentence-features_part*.{fmt}")
    ]
    if module == "syntax":
        glob += [
            *(Path(directory) / module).resolve().glob(f"token-features_part*.{fmt}")
        ]
    ...
    if len(glob) == 0:
        ...
        sentspace.utils.io.log(
            f"loading processed [{module}] data for [{corpus}] in [{directory}]"
        )
        for path in glob:
            if "pkl" in fmt:
        ...
        dfs["corpus"] = corpus
        dfs["sentence_length"] = dfs["sentence"].apply(
            lambda x: len(sentspace.Sentence(x, warn=False))
        )
        if subsample:
            return dfs.sample(frac=0.3)
        return dfs


@functools.lru_cache(maxsize=5)
def load_benchmarks(
    module="lexical", subsample=False, load_gpt_human=True, load_corpora=True
):
    df = pd.DataFrame()
    ...
    if load_gpt_human:
        ...
        df = df.append(
            load_df_from_directory(
                "human_stories",
                relprefix / "out/gpt2stories_sents_target1",
                module=module,
                subsample=False,
            )
        )
    if load_corpora:
        df = df.append(
            load_df_from_directory(
                "brown",
                relprefix
                / "out/benchmarks/brown_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        df = df.append(
            load_df_from_directory(
                "torontoadv",
                relprefix
                / "out/benchmarks/torontoadv_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        df = df.append(
            load_df_from_directory(
                "wsj",
                relprefix
                / "out/benchmarks/wsj_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        df = df.append(
            load_df_from_directory(
                "ud",
                relprefix
                / "out/benchmarks/ud_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        df = df.append(
            load_df_from_directory(
                "c4",
                relprefix
                / "out/benchmarks/c4_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        ...
        df = df.append(
            load_df_from_directory(
                "cocaspok2001",
                relprefix
                / "out/benchmarks/cocaspok2001_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        df = df.append(
            load_df_from_directory(
                "cocaspok2012",
                relprefix
                / "out/benchmarks/cocaspok2012_subsampled_grouped_by_length_n=500_stimuli",
                module=module,
                subsample=subsample,
            )
        )
        ...
        html.Div(
            children=[
                ...
                dcc.Dropdown(
                    id=f"plot-type",
                    options=[
                        {"label": i, "value": i} for i in ("histogram", "scatter")
                    ],
                    value="histogram",
                ),
            ],
    ...
    return html.Div(
        children=[
            ...
            *[
                ...
                for which, init_value in zip(
                    "xyz",
                    [
                        *np.random.choice(df.columns, 3),
                        "surprisal-4",
                        "aoa",
                        "lexical_decision_RT",
                    ],
...
BUTTON_ROW = html.Div(
    children=[html.Hr(), PLOTTING_DROPDOWNS(load_benchmarks()), FILE_UPLOAD],
...
def parse_df(data, filename) -> pd.DataFrame:
    ...
    if filename.endswith("tsv"):
        ...
        temp_df = pd.read_pickle(
            f, compression="gzip" if filename.endswith("pkl.gz") else None
        )
...
def update_graph(
    ...
    if upload_contents and filename:
        ...
        uploaded_df["sentence_length"] = uploaded_df["sentence"].apply(
            lambda x: len(sentspace.Sentence(x, warn=False))
        )
    ...
    if plot_type == "histogram":
        ...
        fig.update_traces(
            marker=dict(size=3, opacity=1, line=dict(width=0.1, color="DarkSlateGray")),
