...
lemmacounts=$(  echo $linetrees  |  python printlemmas.py  |  LC_COLLATE=C sort  |  LC_COLLATE=C uniq -c  |  sed 's/^ *//' )
echo $linetrees  |  ./linetrees2mlpsemprocdecpars -u0 -c0 $lemmacounts > decpars.tmp
echo $input | sed 's/(-NONE-[^)]*)//g' |  sed 's/([^ ]* //g;s/)//g'  |  sed 's/  */ /g;s/^ *//;s/ *$//;'  |  sed 's/!unf! *//g' | python3 calcEmbd.py decpars.tmp
rm decpars.tmp
